...
var eData,nData, nodes=[], links=[],link,node,simulation,timeline,circle;
var dataFolder = "../../data/";
...
d3.csv(dataFolder+"ADVANCE_Outcome_AuthorPublication_EdgeList.csv").then( function(edgeData) {
  d3.csv(dataFolder+"ADVANCE_Outcome_Publications.csv").then( function(pubData) {
    d3.csv(dataFolder+"ADVANCE_Outcome_CoAuthor_AuthorInfo.csv").then( function(authorData) {
      ...
      nData.forEach(function(d){
        nodes.push({radius:sizeScale(d.cite),x:width/2,y:height/2,id:d.id,type:d.type,date:d.datef,name:d.name,cite:d.cite,r:xGraph(d.date)/2});
      });
...
function back2circle_y(d){
  ...
  var newy = d.y - height/2
  var polar = cartesian2polar(newx, newy);
  // console.log(d.dater, d.r, d.date, d.datef, );
  var cart = polar2cartesian(xGraph(d.date)/2, polar.theta);
...
function toggleTimeline(){
  if(timeline){
    ...
    timeline = true;
    simulation.on("tick", fixedradiusticked);
  }
